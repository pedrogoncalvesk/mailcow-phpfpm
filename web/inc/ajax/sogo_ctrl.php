...
if ($_GET['ACTION'] == "start") {
  $retry = 0;
  while (docker('sogo', 'info')['State']['Running'] != 1 && $retry <= 3) {
    $response = docker('sogo', 'post', 'start');
...
if ($_GET['ACTION'] == "stop") {
  $retry = 0;
  while (docker('sogo', 'info')['State']['Running'] == 1 && $retry <= 3) {
    $response = docker('sogo', 'post', 'stop');
    $response = json_decode($response, true);
    $last_response = ($response['type'] == "success") ? '<b><span class="pull-right text-success">OK</span></b>' : '<b><span class="pull-right text-danger">Error: ' . $response['msg'] . '</span></b>';
    if ($response['type'] == "success") {
      break;
...
if ($_GET['ACTION'] == "restart-nginx") {
  
  if (docker('nginx', 'info')['State']['Running'] == 1) {
    $response = docker('nginx', 'post', 'restart');
    $response = json_decode($response, true);
    $last_response = ($response['type'] == "success") ? '<b><span class="pull-right text-success">OK</span></b>' : '<b><span class="pull-right text-danger">Error: ' . $response['msg'] . '</span></b>';
  }
